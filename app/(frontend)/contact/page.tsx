...
const locations = [
  ...
  { name: 'Downtown Clinic', address: '456 Park Avenue, New York, NY 10022', phone: '+1 (800) 123-4568', hours: 'Mon–Fri: 8am–8pm' },
  { name: 'Brooklyn Center', address: '789 Atlantic Ave, Brooklyn, NY 11217', phone: '+1 (800) 123-4569', hours: 'Mon–Sat: 9am–6pm' },
]
    ...
    <div className="bg-white dark:bg-gray-950">
      ...
      <section className="py-16 px-4">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-5 mb-16">
          {[
            { icon: Phone, color: 'bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400', title: 'Phone', lines: ['+1 (800) 123-4567', 'Available 24/7'] },
            { icon: Mail, color: 'bg-green-50 dark:bg-green-950/50 text-green-600 dark:text-green-400', title: 'Email', lines: ['info@medicore.hospital', 'We reply within 24 hours'] },
            ...
            <div key={title} className="flex items-start gap-4 p-6 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <h3 className="font-display font-semibold text-gray-900 dark:text-white mb-1">{title}</h3>
                {lines.map((l, i) => <p key={i} className={`text-sm ${i === 0 ? 'text-gray-700 dark:text-gray-300 font-medium' : 'text-gray-400 dark:text-gray-500'}`}>{l}</p>)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" required value={form.name} onChange={e => update('name', e.target.value)} placeholder="John Doe"
                    ...
                    <div>
                      ...
                      <input type="email" required value={form.email} onChange={e => update('email', e.target.value)} placeholder="john@example.com"
                        className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-gray-100 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </div>
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" required value={form.subject} onChange={e => update('subject', e.target.value)} placeholder="How can we help?"
                        className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-gray-100 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Message *</label>
                    <textarea required value={form.message} onChange={e => update('message', e.target.value)} rows={5} placeholder="Tell us more about your inquiry..."
          ...
          {/* Locations */}
          <div>
            <h2 className="font-display text-2xl font-bold text-gray-900 dark:text-white mb-6">Our Locations</h2>
      ...
      <section className="py-16 px-4 bg-gray-50 dark:bg-gray-900/50">
        <div className="max-w-3xl mx-auto">
          <h2 className="font-display text-2xl font-bold text-gray-900 dark:text-white text-center mb-8">Office Hours</h2>
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm">
            {[
              { day: 'Monday – Friday', hours: '8:00 AM – 8:00 PM', status: 'Open' },
              { day: 'Saturday', hours: '9:00 AM – 6:00 PM', status: 'Open' },
              { day: 'Sunday', hours: '10:00 AM – 4:00 PM', status: 'Limited' },
              { day: 'Emergency Care', hours: '24 hours / 7 days', status: 'Always Open' },
            ].map(({ day, hours, status }, i) => (
              <div key={day} className={`flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 last:border-0 ${i % 2 === 1 ? 'bg-gray-50 dark:bg-gray-800/50' : ''}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                    status === 'Always Open' ? 'bg-green-100 dark:bg-green-950 text-green-600 dark:text-green-400' :
                    status === 'Open' ? 'bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400' :
