...
export default function NursesPage() {
  ...
  return (
    <div className="flex flex-col gap-8 p-5 min-h-screen bg-gray-50 dark:bg-gray-950">
      <Header 
        title="Nurses" 
        subtitle={`${nurses.length} nurses on staff`} 
      />
      ...
      {pendingApplications.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-soft border border-gray-200 dark:border-gray-800 p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Pending Nurse Applications</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Review and approve new nurses</p>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-soft border border-gray-200 dark:border-gray-800 p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Current Nurses</h2>
          <div className="text-sm text-gray-500 dark:text-gray-400">{nurses.length} active</div>
        </div>
                ...
                <div className="flex items-start gap-4">
                  {nurse.photoDataUrl ? (
                    ...
                    <div className="w-20 h-20 rounded-2xl bg-linear-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-3xl font-semibold">
                      {nurse.fullName.split(' ').map(n => n[0]).join('').toUpperCase()}
                    </div>
                  )}

                  <div className="flex-1">
                    <h3 className="font-semibold text-xl text-gray-900 dark:text-white">{nurse.fullName}</h3>
                    <p className="text-brand-600 dark:text-brand-400 font-medium">{nurse.department}</p>
                    <div className="mt-3 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                      <span>{nurse.yearsOfExperience} years</span>
                      <span>•</span>
                      <span>{nurse.school}</span>
                    </div>
                  </div>
                </div>
